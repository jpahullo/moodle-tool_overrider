...
/**
 * Invoke this class in config.php to override Moodle settings.
 *
 * @package   tool_overrider
 * @author    Jordi Pujol Ahulló <jordi.pujol@urv.cat>
 * @copyright 2024 onwards to Universitat Rovira i Virgili (https://www.urv.cat)
...
class settings_overrider {
    /**
     * Invoke this API from config.php before setup.php to override any Moodle setting.
     *
     * The provided $cfg object is populated with overridden settings from the configuration file.
    ...
    public static function settings_from(stdClass $cfg, string $filepath, array $customloaders = []): void {
        ...
        // Add loader class information to tool_overrider metadata.
        if (!isset($settings['tool_overrider'])) {
            $settings['tool_overrider'] = [];
        }
        $settings['tool_overrider']['loader'] = get_class($loader);
